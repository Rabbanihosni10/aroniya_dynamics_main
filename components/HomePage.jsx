...
export default function HomePage() {
  ...
  return (
    <main className="overflow-hidden bg-background text-foreground">
        ...
        <div id="top" className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 items-center px-6 pb-20 pt-14 lg:px-10 lg:pt-4">
          <div className="max-w-4xl">
            <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
              ...
              <motion.p variants={fadeUp} className="mt-8 max-w-xl text-pretty text-base leading-7 text-muted-foreground sm:text-lg">Premium software consultancy for ambitious teams building what&apos;s next. From Bangladesh to the Canada, we make complexity feel simple.</motion.p>
              <motion.div variants={fadeUp} className="mt-10 flex flex-col gap-3 sm:flex-row"><a href="/contact" className="glow-button inline-flex items-center justify-center gap-3 rounded-full bg-primary px-6 py-3.5 text-sm font-semibold text-primary-foreground">Get a free consultation <ArrowUpRight size={17} /></a><a href="/portfolio" className="inline-flex items-center justify-center gap-3 rounded-full border border-border px-6 py-3.5 text-sm font-semibold transition-colors hover:border-primary/70 hover:text-primary">View our work <span className="text-muted-foreground">↘</span></a></motion.div>
            </motion.div>
          </div>
        </div>
        <div className="relative z-10 mx-auto flex w-full max-w-7xl items-center justify-between px-6 pb-8 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground lg:px-10"><span>Dhaka / Canada</span><span className="hidden sm:block">Scroll to explore <span className="ml-3 text-primary">↓</span></span><span>Est. 2021</span></div>
      </section>
      ...
      <footer className="border-t border-border/70"><div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 sm:grid-cols-2 lg:grid-cols-4 lg:px-10"><div><a href="#top" className="flex items-center gap-3"><span className="flex size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Sparkles size={15} /></span><span className="font-mono text-xs font-semibold tracking-[0.16em]">ARONIA<span className="text-primary">.</span></span></a><p className="mt-5 max-w-xs text-sm leading-6 text-muted-foreground">A premium software consultancy for the bold and curious.</p></div><div><p className="footer-label">Explore</p><div className="mt-5 flex flex-col gap-3 text-sm text-muted-foreground"><a href="#services" className="hover:text-foreground">Services</a><a href="/portfolio" className="hover:text-foreground">Portfolio</a><a href="/contact" className="hover:text-foreground">Contact</a></div></div><div><p className="footer-label">Connect</p><div className="mt-5 flex flex-col gap-3 text-sm text-muted-foreground"><span>Dhaka, Bangladesh</span><span>New York, Canada</span><a href="mailto:hello@aroniadynamics.com" className="hover:text-primary">hello@aroniadynamics.com</a></div></div><div><p className="footer-label">Social</p><div className="mt-5 flex gap-3"><a aria-label="Instagram" href="#" className="social-link"><Globe2 size={16} /></a><a aria-label="LinkedIn" href="#" className="social-link"><ArrowUpRight size={16} /></a><a aria-label="Facebook" href="#" className="social-link"><Globe2 size={16} /></a></div></div></div><div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 border-t border-border/70 px-6 py-6 font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground sm:flex-row lg:px-10"><span>© 2026 Aronia Dynamics. All rights reserved.</span><span>Built for what&apos;s next.</span></div></footer>
    </main>
  )
}
